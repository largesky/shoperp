...
{
    class OrderReturnOutPrintDocument : DocumentPaginator, IDocumentPaginatorSource
    {
        ...
        public override DocumentPage GetPage(int pageNumber)
        {
            ...
            int currentPageCount = (pageNumber + 1) == this.PageCount
                ? this.GoodsCount.Length % this.rowPerPage
                : this.rowPerPage;
            ...
            for (int i = 0; i < this.rowPerPage && this.rowPerPage * pageNumber + i < this.GoodsCount.Length; i++)
            {
                ...
                if (DateTime.Now.Subtract(gc.FirstPayTime).TotalHours >= 48)
                {
                    var dateText = this.CreateText(gc.FirstPayTime.ToString("dd"));
                    rendor.DrawText(dateText,
            ...
            rendor.Close();
            return new DocumentPage(visual, this.PageSize, new System.Windows.Rect(this.PageSize),
