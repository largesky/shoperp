    ...
    class PddPop : PopBase
    {
        ...
        public override OrderDownload GetOrder(Domain.Shop shop, string popOrderId)
        {
            ...
            try
            {
                ...
                //获取订单状态
                var os = ConvertToOrderState(popOrderId, new PddRspGetOrderStateOrder { orderSn = popOrderId, order_status = o.order_status, refund_status = o.refund_status });
                order.State = os;
        ...
        public override string[] AddGoods(Shop shop, PopGoods[] popGoodss, float[] buyInPrices)
        {
            ...
            //获取所有地址信息
            para.Clear();
            var addressNode = Invoke<PddRspAddress>(shop, "pdd.logistics.address.get", new SortedDictionary<string, string>());
